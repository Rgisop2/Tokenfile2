...
from helper_func import subscribed, encode, decode, get_messages, get_shortlink, get_verify_status, update_verify_status, get_exp_time, get_verification_image
...
async def start_command(client: Client, message: Message):
    ...
    if id == owner_id:
        ...
        if "verify_" in message.text:
            ...
            # Check if the token is the one we are expecting for the current step
            if verify_status['verify_token'] != token:
                return await message.reply("Your token is invalid or Expired. Try again by clicking /start")
            
            # Check if the token is still valid (not expired)
            now = time.time()
            if verify_status['current_step'] == 0 and now > verify_status['verify1_expiry']:
                return await message.reply("Your first verification token has expired. Try again by clicking /start")
            if verify_status['current_step'] == 1 and now > verify_status['verify2_expiry']:
                return await message.reply("Your second verification token has expired. Try again by clicking /start")
            
            step = verify_status['current_step']
            
            # STEP 1 verification
            if step == 0 and token == verify_status['verify_token']:
                # Set expiry for first verification
            ...
            # STEP 2 verification
                ...
                # Set expiry for second verification
        ...
        # Expiry checks and state reset
        if step == 2 and verify_status['verify2_expiry'] > 0 and now >= verify_status['verify2_expiry']:
            # Second verification expired → reset everything to step 0
            verify_status['is_verified'] = False
            verify_status['current_step'] = 0
            verify_status['verify1_expiry'] = 0
            verify_status['verify2_expiry'] = 0
            verify_status['gap_expiry'] = 0
            await update_verify_status(id, is_verified=False, current_step=0, verify1_expiry=0, verify2_expiry=0, gap_expiry=0)
            step = 0

        elif step == 1 and verify_status['verify1_expiry'] > 0 and now >= verify_status['verify1_expiry']:
            # First verification expired → reset everything to step 0
            verify_status['is_verified'] = False
            verify_status['current_step'] = 0
            verify_status['verify1_expiry'] = 0
            verify_status['gap_expiry'] = 0
            ...
            if step == 2:
                ...
                if verify_status['verify2_expiry'] > 0 and now < verify_status['verify2_expiry']:
                    ...
                    # This case should be handled by the expiry check above, but as a fallback:
                ...
                if verify_status['verify1_expiry'] > 0 and now >= verify_status['verify1_expiry']:
                    ...
                    # Dual verification disabled or gap not set, allow access (effectively full)
                    access_allowed = True
                    access_type = 'full'
            ...
            if not access_allowed and IS_VERIFY and access_type:
                if access_type == 'require_step1':
                    ...
                    # Set a temporary expiry for the token itself to prevent old tokens from being used
                    verify_status['verify1_expiry'] = now + VERIFY_EXPIRE_1
                    await update_verify_status(id, verify_token=token, is_verified=False, current_step=0, verify1_expiry=verify_status['verify1_expiry'])
                    ...
                    if link and isinstance(link, str) and link.startswith(('http://', 'https://', 'tg://')):
                        ...
                        verify_image = await get_verification_image(file_id)
                    ...
                    token = ''.join(random.choices(string.ascii_letters + string.digits, k=10))
                    # Set a temporary expiry for the token itself
                    verify_status['verify2_expiry'] = now + VERIFY_EXPIRE_2
                    await update_verify_status(id, verify_token=token, is_verified=False, current_step=1, verify2_expiry=verify_status['verify2_expiry'])
            ...
            verify_status = await get_verify_status(id)
            # If user is not verified and no deep-link was used, initiate step 1 verification
            if IS_VERIFY and not verify_status['is_verified']:
                token = ''.join(random.choices(string.ascii_letters + string.digits, k=10))
                # Set expiry for the token itself
                verify_status['verify1_expiry'] = now + VERIFY_EXPIRE_1
                await update_verify_status(id, verify_token=token, link="", current_step=0, verify1_expiry=verify_status['verify1_expiry'])
                link = await get_shortlink(SHORTLINK_URL_1, SHORTLINK_API_1, f'https://telegram.dog/{client.username}?start=verify_{token}')
                ...
                if link and isinstance(link, str) and link.startswith(('http://', 'https://', 'tg://')):
                    ...
                    file_id = verify_status.get('link', '')
                    verify_image = await get_verification_image(file_id, is_batch=True)
