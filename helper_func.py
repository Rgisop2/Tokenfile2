...
async def get_verification_image(file_id: str = "", is_batch: bool = False) -> str:
    """Get verification image based on priority: file_image > batch_image > DEFAULT_VERIFY_IMAGE"""
    if file_id:
        try:
            link = await db_get_link(file_id)
            if link:
                # Priority 1: Custom image for single file
                if link.get('image'):
                    return link['image']
                # Priority 2: Batch image
                if is_batch and link.get('batch_image'):
